...
public class DonutControl : MonoBehaviour
{
    ...
    public GameObject Donut_Counter;
    ...
    void Start()
    {
        ...
        transform.Find("kapanis").GetComponent<ParticleSystem>().Stop();
        ...
        basilimi = false;
        transform.Find("Donut_Counter").GetComponent<TextMeshPro>().enabled = false;
    ...
    void OnCollisionStay2D(Collision2D other)
    {
        ...
        {
            BuObje.transform.Find("Red_Alert_Donut").GetComponent<Animator>().SetBool("red_start_donut", true);
        }
        else
        {
            BuObje.transform.Find("Red_Alert_Donut").GetComponent<Animator>().SetBool("red_start_donut", false);
        }
    ...
    void CollisionWork(Collision2D other)
    {
        ...
        //else if (isReady && BuObje.transform.position.y >= 4)
        //{
        //    print("game over");
        //    GameObject.Find("You_Lost").GetComponent<MainStarAnimes>().MoveToLost(new Vector3(0, 0, 0));
        //}
        else
        {
            if (other.gameObject.tag == "donut")
            {
                if (basilimi)
                {
                    if (other.gameObject.GetComponent<DonutControl>().Explode_Counter > 1)
                    {
                        Explode_Counter = Explode_Counter + other.gameObject.GetComponent<DonutControl>().Explode_Counter;
                    }
                    else
                    {
                        Explode_Counter++;
                    ...
                    transform.Find("Donut_Counter").GetComponent<TextMeshPro>().text = Explode_Counter.ToString();
                    ...
                    transform.Find("Donut_Counter").GetComponent<TextMeshPro>().enabled = true;
        ...
        IEnumerator WaitAndDestroy()
        {
            ...
            if (!transform.Find("kapanis").GetComponent<ParticleSystem>().isPlaying)
            {
                transform.Find("kapanis").GetComponent<ParticleSystem>().Play();
            }
            yield return new WaitForSeconds(0.5f); // 1 saniye bekle
            GameObject.Find("GameObject").GetComponent<Stats>().GeneralExpFoods++;
            GameObject.Find("GameObject").GetComponent<Stats>().ExpDonut++;
            Destroy(BuObje);
